...
struct status{
    ...
    int heur()const{
        ...
        for(int i=0;i<box.size();i++)
            h+=target_dist[i][box[i].x][box[i].y]*target_dist[i][box[i].x][box[i].y];
        return h;
    }
    void print(){
        printf("%d %d\n",player.x,player.y);
...
bool lock(status &s){
    bool bb[MAXL+2][MAXL+2];
    bool bx[MAXL+2][MAXL+2];
    bool tg[MAXL+2][MAXL+2];
    ...
    for(int i=0;i<s.box.size();i++){
        ...
        bx[s.box[i].x][s.box[i].y]=true;
        tg[target[i].x][target[i].y]=true;
    }
    for(int i=0;i<n+1;i++)for(int j=0;j<m+1;j++){
        if(!bb[i][j]&&!bb[i+1][j]&&!bb[i][j+1]&&!bb[i+1][j+1]){
            if(bx[i][j]&&!tg[i][j])return true;
            if(bx[i+1][j]&&!tg[i+1][j])return true;
            if(bx[i][j+1]&&!tg[i][j+1])return true;
            if(bx[i+1][j+1]&&!tg[i+1][j+1])return true;
        }
    }
    return false;
    return false;
}
status move(status &s,coord &dir,bool &push){
    status s1=s;
...
int main(){
    ...
    while(scanf("%d%d",&n,&m)!=EOF){
        ...
        while(true){
            num_s++;
            if(tc<10&&num_s>(int)8e5){
                break;
            }
            if(Q.size()<unQ.size()&&!Q.empty()){
                int hh=Q.top().first;
